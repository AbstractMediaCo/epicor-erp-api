...
class ServiceBase {
  ...
  makeRequest(method, parameters, company, props) {
  ...
  /**
   * Update a record.  The record must have the original values from the record,
   * because Epicor will throw an error if some of the fields are missing.
   */
  update(record) {
   ...
   */
  find(where = '', { pageSize = 25, limit = 0 } = {}, path = 'GetList', reqProps = {}) {
  ...
  /**
   * Low-level method for requesting a page of rows. Depending on the service,
   * this may need to use GetList or the more complex GetRows method.
   * This can be overridden by subclasses to implement custome GetRows method.
   */
  _getRows(whereClause, pageSize, absolutePage, reqProps) {
